...
error_reporting(0);
session_start();
// if (strlen($_SESSION['user_id']) == 0 && strlen($_SESSION['adm_id']) == 0) {
...
if (isset($_POST['update'])) {
  ...
  $status = $_POST['status'];
  $remark = $_POST['remark'];
  $chkform = mysqli_query($db, "select * from appointment_remark where frm_id= '$form_id'");
  if(mysqli_num_rows($chkform) == 0)
  {
    $query = mysqli_query($db, "insert into appointment_remark(frm_id,status,remark) values('$form_id','$status','$remark')");
    $sql = mysqli_query($db, "update appointment_list set status='$status' where id='$form_id'");
  }

  else 
  {
    $query = mysqli_query($db, "update appointment_remark set status='$status', remark='$remark' where frm_id='$form_id'");
    $sql = mysqli_query($db, "update appointment_list set status='$status' where id='$form_id'");
  }
